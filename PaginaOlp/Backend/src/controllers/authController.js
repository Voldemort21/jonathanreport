...
const { findByEmail, save,findByUsername } = require('../models/userModel');
...
exports.register = async (req, res) => {
  try {
    ...
    let user = await findByEmail(email); // esta buscando al usuario por email
    let user_1= await findByUsername(username); // esta buscando el usuario por nombre
    if (user) {
      return res.status(400).json({ message: 'El usuario ya existe ' });
    }
    if (user_1) {
      return res.status(400).json({ message: 'El usuario ya existe, intenta con otro nombre' });
    }
